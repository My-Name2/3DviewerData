...
def read_uploaded_csv(
    ...
    first_col_is_id: bool = True,
) -> pd.DataFrame:
    """Read an uploaded CSV with Stata-like header handling.

    If first_row_headers is False, the first row is treated as data and
    generic column names are created.
    """
    ...
    if first_col_is_id and raw.shape[1] >= 1:
        raw.columns = ["ticker"] + [f"var{i}" for i in range(1, raw.shape[1])]
    else:
        raw.columns = [f"var{i}" for i in range(1, raw.shape[1] + 1)]
...
with st.sidebar:
    ...
    first_row_headers = st.checkbox(
        "First row contains column names",
        value=True,
        help="Like Stata's header/variable names option. Turn this off if row 1 is actual data.",
    )
    first_col_is_id = st.checkbox(
        "First column is ticker / ID",
        value=True,
        help="Only used when the file has no headers. If checked, column 1 is named ticker.",
    )
...
if uploaded_file is not None:
    df = read_uploaded_csv(
        ...
        first_col_is_id=first_col_is_id,
    )
else:
    df = load_data()
